...
public class BiblioCraftTweakProvider implements TweakProvider {
    ...
    public IInventory getCraftMatrix(EntityPlayer entityPlayer, Container container, int id) {
        try {
            return (IInventory) playerCraftMatrixField.get(container);
        } catch (IllegalAccessException e) {
            e.printStackTrace();
            return null;
        }
    ...
    public void initGui(GuiContainer guiContainer, List buttonList) {
        ...
        buttonList.add(CraftingTweaksAPI.createRotateButton(0, guiContainer.width / 2 - 104, guiContainer.guiTop + paddingTop));
        buttonList.add(CraftingTweaksAPI.createBalanceButton(0, guiContainer.width / 2 - 104, guiContainer.guiTop + paddingTop + 18));
        buttonList.add(CraftingTweaksAPI.createClearButton(0, guiContainer.width / 2 - 104, guiContainer.guiTop + paddingTop + 36));
    }
