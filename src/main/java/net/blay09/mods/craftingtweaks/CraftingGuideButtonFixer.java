...
public class CraftingGuideButtonFixer {
	...
	private static GuiButton findCraftButton(List<GuiButton> buttonList) {
		return buttonList.stream().filter(p -> p instanceof GuiButtonImage && ((GuiButtonImage) p).resourceLocation.getResourcePath().equals("textures/gui/container/crafting_table.png")).findFirst().orElse(null);
	}
